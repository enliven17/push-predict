      ...
      <div className="max-w-7xl mx-auto flex flex-col items-center gap-4">
        <Link href="/" className="flex items-center gap-2">
          <span className="text-lg sm:text-xl font-bold tracking-tight">
            <span className="bg-gradient-to-r from-[#e91e63] via-[#f06292] to-[#ad1457] bg-clip-text text-transparent">Push</span>
        ...
        <nav className="flex flex-wrap justify-center gap-x-5 gap-y-2 text-xs sm:text-sm text-gray-400">
          {footerSections.flatMap((s) => s.links).map((link) => (
            <Link
              key={link.label}
              href={link.href}
              className="hover:text-[#e91e63] transition-colors duration-200"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="w-full flex items-center justify-between text-[11px] sm:text-xs text-gray-500">
          <span>© {new Date().getFullYear()} PushPredict</span>
